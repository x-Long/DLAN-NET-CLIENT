...
(function (global, factory) {
    ...
    function initRender(vm) {
        ...
        var parentVnode = vm.$vnode = vm.$options
        ._parentVnode; // the placeholder node in parent tree
            ...
            if (isDef(tag)) {
                {
                    ...
                    if (
                        !inPre &&
                        !vnode.ns &&
                        !(config.ignoredElements.length && config.ignoredElements.indexOf(tag) > -
                        1) &&
        ...
        function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
            ...
            while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
                if (isUndef(oldStartVnode)) {
                    ...
                    canMove && nodeOps.insertBefore(parentElm, oldStartVnode.elm, nodeOps
                    ...
                    if (isUndef(idxInOld)) { // New element
                        ...
                        if (sameVnode(elmToMove, newStartVnode)) {
                            ...
                            canMove && nodeOps.insertBefore(parentElm, elmToMove.elm, oldStartVnode
                                .elm);
                            newStartVnode = newCh[++newStartIdx];
                        } else {
    ...
    var Transition = {
        ...
        render: function render(h) {
            ...
            var data = (child.data || (child.data = {})).transition = extractTransitionData(
                this);
            ...
            for (var i = 0; i < rawChildren.length; i++) {
                ...
                if (c.tag) {
                    if (c.key != null && String(c.key).indexOf('__vlist') !== 0) {
                        ...
                        var name = opts ? (opts.Ctor.options.name || opts.tag || '') : c
                        .tag;
        ...
        updated: function updated() {
            ...
            children.forEach(function (c) {
                if (c.data.moved) {
                    ...
                    el.addEventListener(transitionEndEvent, el._moveCb =
                        function cb(e) {
    ...
    function parse(
        ...
        parseHTML(template, {
            ...
            chars: function chars(text) {
                ...
                if (text) {
                    var expression;
                    if (!inVPre && text !== ' ' && (expression = parseText(text,
                            delimiters))) {
                        children.push({
                            type: 2,
                    ...
                    } else if (text !== ' ' || !children.length || children[children
                            .length - 1].text !== ' ') {
                        children.push({
                            type: 3,
                            text: text
    ...
    function genKeyFilter(keys) {
        return ("if(!('button' in $event)&&" + (keys.map(genFilterCode).join('&&')) +
            ")return null;")
    }
    ...
    function genFilterCode(key) {
        ...
        return ("_k($event.keyCode," + (JSON.stringify(key)) + (alias ? ',' + JSON.stringify(
            alias) : '') + ")")
    }
    ...
    function bind$1(el, dir) {
        el.wrapData = function (code) {
            return ("_b(" + code + ",'" + (el.tag) + "'," + (dir.value) + "," + (dir
                .modifiers && dir.modifiers.prop ? 'true' : 'false') + (dir.modifiers &&
                dir.modifiers.sync ? ',true' : '') + ")")
        };
    }
    ...
    function genDirectives(el, state) {
        ...
        for (i = 0, l = dirs.length; i < l; i++) {
            ...
            if (needRuntime) {
                hasRuntime = true;
                res += "{name:\"" + (dir.name) + "\",rawName:\"" + (dir.rawName) + "\"" + (dir
                    .value ? (",value:(" + (dir.value) + "),expression:" + (JSON.stringify(dir
                        .value))) : '') + (dir.arg ? (",arg:\"" + (dir.arg) + "\"") : '') + (dir
                ...
                if (typeof template === 'string') {
                    if (template.charAt(0) === '#') {
                        ...
                        if ("development" !== 'production' && !template) {
                            warn(
                                ("Template element not found or is empty: " + (options
                                    .template)),
                                this
                            );
                        }
